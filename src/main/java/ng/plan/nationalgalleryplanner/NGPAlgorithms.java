...
public class NGPAlgorithms {
    public static <T> List<GraphNodeAdjList<Room>> findPathDepthFirst(GraphNodeAdjList<Room> from, List<GraphNodeAdjList<Room>> encountered, Room lookingfor, List<GraphNodeAdjList<Room>> avoid){
        ...
        for (GraphNodeAdjList<Room> avoidRoom : avoid){ //checking if current node is a room to avoid
            if (from.data.equals(avoidRoom.data)){
                return null;            //if room is to be avoided, return null (stop path from progressing down the link)
            }
        }
        ...
        for(GraphLinkAdjList adjLink : from.adjList)
            if(!encountered.contains(adjLink.destNode)) {
                result=findPathDepthFirst((GraphNodeAdjList<Room>) adjLink.destNode, encountered,lookingfor, avoid);
    ...
    public static <T> List<List<GraphNodeAdjList<Room>>> findAllPathsDepthFirst(GraphNodeAdjList<Room> from, List<GraphNodeAdjList<Room>> encountered, Room lookingfor, List<GraphNodeAdjList<Room>> avoid){
        ...
        for (GraphNodeAdjList<Room> avoidRoom : avoid){
            if (from.data.equals(avoidRoom.data)){
                return null;
            }
        }
        ...
        for(GraphLinkAdjList adjLink : from.adjList){
            if(!encountered.contains(adjLink.destNode)) {
                temp2=findAllPathsDepthFirst((GraphNodeAdjList<Room>) adjLink.destNode,new ArrayList<>(encountered),lookingfor, avoid); //Use clone of encountered list for recursive call!
    ...
    public static <T> CostedPath findCheapestPathDijkstra(GraphNodeAdjList<Room> startNode, T lookingfor, List<GraphNodeAdjList<Room>> avoid) {
    ...
    public static <T> List<GraphNodeAdjList<Room>> findPathBreadthFirst(GraphNodeAdjList<Room> startNode, T lookingfor, List<GraphNodeAdjList<Room>> avoid){
    ...
    public static <T> List<GraphNodeAdjList<Room>> findPathBreadthFirst(List<List<GraphNodeAdjList<Room>>> agenda, List<GraphNodeAdjList<Room>> encountered, T lookingfor, List<GraphNodeAdjList<Room>> avoid){
        ...
        for (GraphNodeAdjList<Room> avoidRoom : avoid){
            if (currentNode.equals(avoidRoom)){     //if room is to be avoided
                return findPathBreadthFirst(agenda,encountered,lookingfor,avoid); //skip adding adjacent nodes to path list
            }
        }
        ...
        return findPathBreadthFirst(agenda,encountered,lookingfor, avoid); //Tail call
    }
}
